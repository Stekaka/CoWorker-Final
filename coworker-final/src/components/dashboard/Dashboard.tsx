      ...
      <GlassCard className="p-6 relative overflow-hidden" variant="subtle">
        <div className="flex items-center justify-between">
          ...
          <div className={`p-3 rounded-xl ${gradient}`}>
            <Icon className={`w-6 h-6 ${isDark ? 'text-white' : 'text-white'}`} />
          </div>
        </div>
      </GlassCard>
    </motion.div>
    ...
    <div className="space-y-8">
      <div className="p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <h1 className={`text-4xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'} mb-2`}>Hej! 👋</h1>
      ...
      <GlassCard className="p-8" variant="subtle">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className={`text-xl font-semibold mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>Försäljningstrend</h3>
            <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Utveckling de senaste 6 månaderna</p>
          </div>
          <div className="flex items-center space-x-2">
            <div className="w-3 h-3 rounded-full bg-blue-400"></div>
            <span className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Försäljning</span>
          </div>
        </div>
        <div className="h-80">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <GlassCard className="p-6" variant="subtle">
          <div className="flex items-center justify-between mb-6">
            <h3 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Senaste kunder</h3>
            ...
            {recentContacts.map((contact) => (
              <div key={contact.id} className="flex items-center space-x-4 p-3 rounded-xl hover:bg-white/[0.02] transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium truncate ${isDark ? 'text-white' : 'text-gray-900'}`}>{contact.name}</p>
                  <p className={`text-xs truncate ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{contact.company || contact.email}</p>
                </div>
        ...
        <GlassCard className="p-6" variant="subtle">
          <div className="flex items-center justify-between mb-6">
            <h3 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Pågående affärer</h3>
            <GlassButton variant="ghost" size="sm">
              Visa alla
            </GlassButton>
          </div>
            ...
            {activeDeals.map((deal) => (
              <div key={deal.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-white/[0.02] transition-colors">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className={`text-sm font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>{deal.title}</p>
                    <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{deal.stage}</p>
                  </div>
                </div>
      ...
      <GlassCard className="p-6" variant="subtle">
        <div className="flex items-center justify-between mb-6">
          <h3 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Att göra idag</h3>
          <GlassButton variant="ghost" size="sm">
          ...
          {upcomingTasks.map((task) => (
            <div key={task.id} className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] transition-colors">
              <div className="flex items-start justify-between mb-2">
                <h4 className={`text-sm font-medium truncate ${isDark ? 'text-white' : 'text-gray-900'}`}>{task.title}</h4>
                {task.priority === 'high' && (
                  <Star className="w-4 h-4 text-orange-400 flex-shrink-0 ml-2" />
                )}
              </div>
              <p className={`text-xs mb-3 line-clamp-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{task.description}</p>
